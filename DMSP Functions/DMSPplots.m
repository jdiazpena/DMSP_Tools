function DMSPplots(InitialTime,FinalTime,data,ticks,mltvar)
...
figure;
s1=subplot(5,1,1);
set(s1, 'Position', get(s1,'Position')-[0 0 0.04 -0.04]);
...
ylabel('Electron Eenergy (eV)','fontsize',fontsize);
...
ylabel('Ion Energy (eV)','fontsize',fontsize);
...
plot(timem,newtime(indexi:indexe),'r.','MarkerSize',6);
hold on
...
axis([min(timem) max(timem) 0 mav]);
